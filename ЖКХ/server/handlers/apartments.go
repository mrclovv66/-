...
func GetApartments(c *gin.Context, db *sql.DB) {
	rows, err := db.Query("SELECT Адрес, Id_владельца, Количество_комнат, Площадь FROM Квартира")
	if err != nil {
	...
	for rows.Next() {
		...
		rows.Scan(&a.Address, &a.ClientID, &a.Rooms, &a.Area)
...
func CreateApartment(c *gin.Context, db *sql.DB) {
	...
	_, err := db.Exec("INSERT INTO Квартира (Адрес, Id_владельца, Количество_комнат, Площадь) VALUES (@p1, @p2, @p3, @p4)",
...
func UpdateApartment(c *gin.Context, db *sql.DB) {
	...
	_, err := db.Exec("UPDATE Квартира SET Id_владельца=@p1, Количество_комнат=@p2, Площадь=@p3 WHERE Адрес=@p4",
...
func DeleteApartment(c *gin.Context, db *sql.DB) {
	...
	_, err := db.Exec("DELETE FROM Квартира WHERE Адрес=@p1", address)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
